...
const Result = () => {
  ...
  const { test, questions, answers, flags, originalQuestions } = location.state || {}; // eslint-disable-line no-unused-vars
  
  console.log("FIRST QUESTION:", originalQuestions?.[0]?.id);
  console.log("LAST QUESTION:", originalQuestions?.[originalQuestions?.length - 1]?.id);
...
// Maps for subject normalization
    const subjectMap = {
      maths: 'maths',
      phy: 'physics',
      che: 'chemistry'
    ...
    originalQuestions.forEach((q, originalIdx) => {
      const userAns = answers?.[originalIdx];
      ...
      const subjectFull = subjectMap[q.subject];
      if (!subjectFull) {
      ...
      const subjectShort = subjectShortMap[q.subject] || q.subject;
      const chapterId = q.chapter; // 'm1'|'p1'|'c1'

      console.log("SUBJECT CHECK:", q.subject, "→ full:", subjectFull, "short:", subjectShort);
      ...
      <Container>
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-8">
          <Button 
            variant="primary" 
            size="lg"
            onClick={() => navigate('/review', { state: { questions: originalQuestions, answers, flags } })}
            className="flex-1"
          >
            Review Questions
          </Button>
