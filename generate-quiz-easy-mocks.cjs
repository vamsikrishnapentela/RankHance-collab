...
// Generate 20 easy tests et1 to et20 from quiz questions
const startMock = 1;
const endMock = 20;
for (let mockNum = startMock; mockNum <= endMock; mockNum++) {
  console.log(`\\nGenerating easy mock et${mockNum}...`);
  ...
  if (!valid || dupCount > 0 || total !== 160) {
    console.error('Validation failed for et' + mockNum);
    continue;
  }
  
  const mockDir = path.join(OUTPUT_DIR, `et${mockNum}`);
  ...
  console.log(`✅ Saved et${mockNum}/data.json (${total} questions)`);
}

console.log('\\n✅ Quiz easy tests complete! et1-et20');
console.log('Total mocks:', metadata.length);
